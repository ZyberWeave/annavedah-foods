'use client'

import { useEffect, useRef, useState, useCallback } from 'react'
import { Instagram, ChevronLeft, ChevronRight } from 'lucide-react'

declare global {
  interface Window {
    instgrm?: { Embeds: { process: () => void } }
  }
}

interface ReelData {
  id: number
  permalink: string
  caption: string | null
}

// Fallback reels if DB is empty
const FALLBACK_REELS: ReelData[] = [
  { id: 1, permalink: 'https://www.instagram.com/reel/DXmEU_TAIpF/?utm_source=ig_embed&utm_campaign=loading', caption: 'A post shared by Annavedah Foods | Healthy Living (@annavedah.foods)' },
  { id: 2, permalink: 'https://www.instagram.com/reel/DXZMShpgsed/?utm_source=ig_embed&utm_campaign=loading', caption: 'A post shared by Annavedah Foods | Healthy Living (@annavedah.foods)' },
  { id: 3, permalink: 'https://www.instagram.com/reel/DXy8Rfgh65K/?utm_source=ig_embed&utm_campaign=loading', caption: 'A post shared by Annavedah Foods | Healthy Living (@annavedah.foods)' },
  { id: 4, permalink: 'https://www.instagram.com/reel/DXwom2qoOeV/?utm_source=ig_embed&utm_campaign=loading', caption: 'A post shared by 𝐀 𝐍 𝐆 𝐀 𝐑 𝐈 𝐊 𝐀 🌺 (@heyitsangarika.17)' },
  { id: 5, permalink: 'https://www.instagram.com/reel/DXBS8PoiFz2/?utm_source=ig_embed&utm_campaign=loading', caption: 'A post shared by Annavedah Foods | Healthy Living (@annavedah.foods)' },
  { id: 6, permalink: 'https://www.instagram.com/reel/DWykxKcDQr2/?utm_source=ig_embed&utm_campaign=loading', caption: 'A post shared by Annavedah Foods | Healthy Living (@annavedah.foods)' },
]

function InstagramEmbedCard({ reel }: { reel: ReelData }) {
  return (
    <div className="instagram-feed-card mx-auto flex w-full min-w-0 max-w-full items-start justify-center overflow-hidden rounded-md bg-white shadow-[0_0_1px_0_rgba(0,0,0,0.5),0_1px_10px_0_rgba(0,0,0,0.15)] md:max-w-[540px]">
      <blockquote
        className="instagram-media"
        data-instgrm-captioned
        data-instgrm-permalink={reel.permalink}
        data-instgrm-version="14"
        style={{
          background: '#FFF',
          border: 0,
          borderRadius: '3px',
          boxShadow: 'none',
          margin: '0 auto',
          maxWidth: '540px',
          minWidth: 0,
          padding: 0,
          width: '100%',
        }}
      >
        <div style={{ padding: '16px' }}>
          <a
            href={reel.permalink}
            style={{ background: '#FFFFFF', lineHeight: 0, padding: '0 0', textAlign: 'center', textDecoration: 'none', width: '100%' }}
            target="_blank"
            rel="noopener noreferrer"
          >
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
              <div style={{ backgroundColor: '#F4F4F4', borderRadius: '50%', flexGrow: 0, height: '40px', marginRight: '14px', width: '40px' }} />
              <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1, justifyContent: 'center' }}>
                <div style={{ backgroundColor: '#F4F4F4', borderRadius: '4px', flexGrow: 0, height: '14px', marginBottom: '6px', width: '100px' }} />
                <div style={{ backgroundColor: '#F4F4F4', borderRadius: '4px', flexGrow: 0, height: '14px', width: '60px' }} />
              </div>
            </div>
            <div style={{ padding: '19% 0' }} />
            <div style={{ display: 'block', height: '50px', margin: '0 auto 12px', width: '50px' }}>
              <svg width="50px" height="50px" viewBox="0 0 60 60" version="1.1" xmlns="http://www.w3.org/2000/svg">
                <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                  <g transform="translate(-511.000000, -20.000000)" fill="#000000">
                    <g>
                      <path d="M556.869,30.41 C554.814,30.41 553.148,32.076 553.148,34.131 C553.148,36.186 554.814,37.852 556.869,37.852 C558.924,37.852 560.59,36.186 560.59,34.131 C560.59,32.076 558.924,30.41 556.869,30.41 M541,60.657 C535.114,60.657 530.342,55.887 530.342,50 C530.342,44.114 535.114,39.342 541,39.342 C546.887,39.342 551.658,44.114 551.658,50 C551.658,55.887 546.887,60.657 541,60.657 M541,33.886 C532.1,33.886 524.886,41.1 524.886,50 C524.886,58.899 532.1,66.113 541,66.113 C549.9,66.113 557.115,58.899 557.115,50 C557.115,41.1 549.9,33.886 541,33.886 M565.378,62.101 C565.244,65.022 564.756,66.606 564.346,67.663 C563.803,69.06 563.154,70.057 562.106,71.106 C561.058,72.155 560.06,72.803 558.662,73.347 C557.607,73.757 556.021,74.244 553.102,74.378 C549.944,74.521 548.997,74.552 541,74.552 C533.003,74.552 532.056,74.521 528.898,74.378 C525.979,74.244 524.393,73.757 523.338,73.347 C521.94,72.803 520.942,72.155 519.894,71.106 C518.846,70.057 518.197,69.06 517.654,67.663 C517.244,66.606 516.755,65.022 516.623,62.101 C516.479,58.943 516.448,57.996 516.448,50 C516.448,42.003 516.479,41.056 516.623,37.899 C516.755,34.978 517.244,33.391 517.654,32.338 C518.197,30.938 518.846,29.942 519.894,28.894 C520.942,27.846 521.94,27.196 523.338,26.654 C524.393,26.244 525.979,25.756 528.898,25.623 C532.057,25.479 533.004,25.448 541,25.448 C548.997,25.448 549.943,25.479 553.102,25.623 C556.021,25.756 557.607,26.244 558.662,26.654 C560.06,27.196 561.058,27.846 562.106,28.894 C563.154,29.942 563.803,30.938 564.346,32.338 C564.756,33.391 565.244,34.978 565.378,37.899 C565.522,41.056 565.552,42.003 565.552,50 C565.552,57.996 565.522,58.943 565.378,62.101 M570.82,37.631 C570.674,34.438 570.167,32.258 569.425,30.349 C568.659,28.377 567.633,26.702 565.965,25.035 C564.297,23.368 562.623,22.342 560.652,21.575 C558.743,20.834 556.562,20.326 553.369,20.18 C550.169,20.033 549.148,20 541,20 C532.853,20 531.831,20.033 528.631,20.18 C525.438,20.326 523.257,20.834 521.349,21.575 C519.376,22.342 517.703,23.368 516.035,25.035 C514.368,26.702 513.342,28.377 512.574,30.349 C511.834,32.258 511.326,34.438 511.181,37.631 C511.035,40.831 511,41.851 511,50 C511,58.147 511.035,59.17 511.181,62.369 C511.326,65.562 511.834,67.743 512.574,69.651 C513.342,71.625 514.368,73.296 516.035,74.965 C517.703,76.634 519.376,77.658 521.349,78.425 C523.257,79.167 525.438,79.673 528.631,79.82 C531.831,79.965 532.853,80.001 541,80.001 C549.148,80.001 550.169,79.965 553.369,79.82 C556.562,79.673 558.743,79.167 560.652,78.425 C562.623,77.658 564.297,76.634 565.965,74.965 C567.633,73.296 568.659,71.625 569.425,69.651 C570.167,67.743 570.674,65.562 570.82,62.369 C570.966,59.17 571,58.147 571,50 C571,41.851 570.966,40.831 570.82,37.631" />
                    </g>
                  </g>
                </g>
              </svg>
            </div>
            <div style={{ paddingTop: '8px' }}>
              <div style={{ color: '#3897f0', fontFamily: 'Arial,sans-serif', fontSize: '14px', fontStyle: 'normal', fontWeight: 550, lineHeight: '18px' }}>
                View this post on Instagram
              </div>
            </div>
            <div style={{ padding: '12.5% 0' }} />
          </a>
          <p style={{ color: '#c9c8cd', fontFamily: 'Arial,sans-serif', fontSize: '14px', lineHeight: '17px', marginBottom: 0, marginTop: '8px', overflow: 'hidden', padding: '8px 0 7px', textAlign: 'center', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            <a href={reel.permalink} style={{ color: '#c9c8cd', fontFamily: 'Arial,sans-serif', fontSize: '14px', fontStyle: 'normal', fontWeight: 'normal', lineHeight: '17px', textDecoration: 'none' }} target="_blank" rel="noopener noreferrer">
              {reel.caption || 'A post shared on Instagram'}
            </a>
          </p>
        </div>
      </blockquote>
    </div>
  )
}

export default function InstagramFeed() {
  const [reels, setReels] = useState<ReelData[]>([])
  const [currentIndex, setCurrentIndex] = useState(0)
  const [isPaused, setIsPaused] = useState(false)
  const sliderRef = useRef<HTMLDivElement>(null)
  const autoPlayRef = useRef<NodeJS.Timeout | null>(null)

  // Fetch reels from API, fallback to hardcoded defaults
  useEffect(() => {
    fetch('/api/instagram-reels')
      .then(res => res.json())
      .then(data => {
        if (data.reels && data.reels.length > 0) {
          setReels(data.reels)
        } else {
          setReels(FALLBACK_REELS)
        }
      })
      .catch(() => setReels(FALLBACK_REELS))
  }, [])

  // Load Instagram embed script
  useEffect(() => {
    if (reels.length === 0) return

    const existingScript = document.querySelector('script[src*="instagram.com/embed.js"]')
    if (!existingScript) {
      const script = document.createElement('script')
      script.src = 'https://www.instagram.com/embed.js'
      script.async = true
      script.onload = () => {
        if (window.instgrm) window.instgrm.Embeds.process()
      }
      document.body.appendChild(script)
    } else if (window.instgrm) {
      // Delay to allow DOM to render
      setTimeout(() => window.instgrm?.Embeds.process(), 500)
    }
  }, [reels])

  // How many visible at once based on breakpoints
  const getVisibleCount = useCallback(() => {
    if (typeof window === 'undefined') return 1
    if (window.innerWidth >= 1024) return 3
    if (window.innerWidth >= 768) return 2
    return 1
  }, [])

  const [visibleCount, setVisibleCount] = useState(1)

  useEffect(() => {
    const handleResize = () => setVisibleCount(getVisibleCount())
    handleResize()
    window.addEventListener('resize', handleResize)
    return () => window.removeEventListener('resize', handleResize)
  }, [getVisibleCount])

  const maxIndex = Math.max(0, reels.length - visibleCount)
  const safeCurrentIndex = Math.min(currentIndex, maxIndex)

  // Auto-play slider
  useEffect(() => {
    if (isPaused || reels.length <= visibleCount) return

    autoPlayRef.current = setInterval(() => {
      setCurrentIndex(prev => (prev >= maxIndex ? 0 : prev + 1))
    }, 6000)

    return () => {
      if (autoPlayRef.current) clearInterval(autoPlayRef.current)
    }
  }, [isPaused, maxIndex, reels.length, visibleCount])

  // Re-process Instagram embeds when slide changes
  useEffect(() => {
    if (window.instgrm) {
      setTimeout(() => window.instgrm?.Embeds.process(), 300)
    }
  }, [currentIndex])

  const goTo = (index: number) => {
    setCurrentIndex(Math.max(0, Math.min(index, maxIndex)))
  }

  const prev = () => goTo(safeCurrentIndex - 1)
  const next = () => goTo(safeCurrentIndex >= maxIndex ? 0 : safeCurrentIndex + 1)

  if (reels.length === 0) return null

  return (
    <section className="py-12 md:py-16 bg-[#faf6f0] overflow-hidden">
      <div className="container mx-auto px-3 sm:px-4">
        <div className="text-center mb-10">
          <p className="text-sm font-semibold uppercase tracking-widest text-[#c9a45c] mb-2 flex items-center justify-center gap-2">
            <Instagram className="w-4 h-4" /> Follow @annavedah.foods
          </p>
          <h2 className="text-3xl md:text-4xl font-bold text-[#2d1b15]">From our community</h2>
          <p className="text-[#6b5347] mt-2 max-w-2xl mx-auto">Real kitchens, real recipes, real wellness journeys with Annavedah.</p>
        </div>

        {/* Auto-sliding Carousel */}
        <div
          className="relative max-w-7xl mx-auto overflow-hidden"
          onMouseEnter={() => setIsPaused(true)}
          onMouseLeave={() => setIsPaused(false)}
        >
          {/* Navigation Arrows */}
          {reels.length > visibleCount && (
            <>
              <button
                onClick={prev}
                className="absolute left-1 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white border-2 border-[#e8ddd0] hover:border-[#c9a45c] shadow-lg flex items-center justify-center transition-all hover:scale-110 md:left-0 md:-translate-x-3"
                aria-label="Previous reel"
              >
                <ChevronLeft className="w-5 h-5 text-[#8b1a1a]" />
              </button>
              <button
                onClick={next}
                className="absolute right-1 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white border-2 border-[#e8ddd0] hover:border-[#c9a45c] shadow-lg flex items-center justify-center transition-all hover:scale-110 md:right-0 md:translate-x-3"
                aria-label="Next reel"
              >
                <ChevronRight className="w-5 h-5 text-[#8b1a1a]" />
              </button>
            </>
          )}

          {/* Slider Track */}
          <div className="overflow-hidden rounded-lg mx-4 sm:mx-6" ref={sliderRef}>
            <div
              className="flex min-w-0 items-stretch transition-transform duration-700 ease-in-out"
              style={{
                transform: `translateX(-${safeCurrentIndex * (100 / visibleCount)}%)`,
              }}
            >
              {reels.map((reel) => (
                <div
                  key={reel.id}
                  className="flex min-w-0 flex-shrink-0 overflow-hidden px-0 sm:px-3"
                  style={{ width: `${100 / visibleCount}%` }}
                >
                  <InstagramEmbedCard reel={reel} />
                </div>
              ))}
            </div>
          </div>

          {/* Dot Indicators */}
          {reels.length > visibleCount && (
            <div className="flex justify-center gap-2 mt-6">
              {Array.from({ length: maxIndex + 1 }).map((_, i) => (
                <button
                  key={i}
                  onClick={() => goTo(i)}
                  className={`h-2 rounded-full transition-all duration-300 ${
                    i === safeCurrentIndex
                      ? 'w-8 bg-[#c9a45c]'
                      : 'w-2 bg-[#e8ddd0] hover:bg-[#c9a45c]/50'
                  }`}
                  aria-label={`Go to slide ${i + 1}`}
                />
              ))}
            </div>
          )}
        </div>

        <div className="text-center mt-8">
          <a
            href="https://www.instagram.com/annavedah.foods?igsh=Zmo5YXA0bzRlbHRm&utm_source=qr"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 border-2 border-[#c9a45c] text-[#8b1a1a] font-bold uppercase tracking-wider text-sm rounded-xl hover:bg-[#c9a45c]/10 transition-colors"
          >
            <Instagram className="w-4 h-4" /> Follow us on Instagram
          </a>
        </div>
      </div>
    </section>
  )
}
